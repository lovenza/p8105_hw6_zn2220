...
```
    ...
    ##    .id   term       estimate
    ##    <chr> <chr>         <dbl>
    ##  1 0001  r.squared     0.911
    ##  2 0001  beta_ratio -531.   
    ##  3 0002  r.squared     0.923
    ##  4 0002  beta_ratio -529.   
    ##  5 0003  r.squared     0.911
    ##  6 0003  beta_ratio -433.   
    ##  7 0004  r.squared     0.915
    ##  8 0004  beta_ratio -617.   
    ##  9 0005  r.squared     0.908
    ## 10 0005  beta_ratio -973.   
    ## # ℹ 9,990 more rows

## Plot the distribution of your estimates, and describe these in words.

``` r
plot_data <- boot_results %>%
  group_by(term) %>%
  filter(term == "r.squared" | (estimate > quantile(estimate, 0.01) & estimate < quantile(estimate, 0.99))) %>%
  ungroup()
...
plot_data %>% 
  ...
  labs(
    ...
    subtitle = "beta_ratio (98% range) vs r.squared (full range)",
    x = NULL,
    y = "Density"
  ) +
...
Left Panel:
$$\hat{\beta}_{\text{ratio}} = \hat{\beta}_1 / \hat{\beta}_2$$

The bootstrap distribution of $\hat{\beta}_{\text{ratio}}$ is highly
leptokurtic (sharp peak) and centered near 0, with extremely heavy tails
extending from approximately –12,000 to +12,000. Unlike the previous
description, the distribution is roughly symmetric and includes both
positive and negative values. This extreme spread suggests that the
denominator (the predictor’s coefficient) is frequently close to zero,
causing the ratio estimate to be highly unstable and explode in
magnitude.

Right Panel: $$r^2$$

The bootstrap distribution of $r^2$ is unimodal and left-skewed, peaking
around 0.915. The values are concentrated roughly between 0.88 and
0.935. These consistently high values demonstrate that the model
explains about 88%–93% of the variation in maximum temperature,
...
|:-----------|----------:|---------:|
| beta_ratio | -5930.581 | 4724.353 |
| r.squared  |     0.894 |    0.928 |

- beta_ratio (Inconclusive / High Instability):  
  The 95% confidence interval for
  $\hat{\beta}_{\text{ratio}} = \hat{\beta}_1 / \hat{\beta}_2$ falls
  between –5930.581 and 4724.353. Because this interval includes zero
  and is extremely wide, the ratio is not statistically significant. The
  span from large negative to large positive values indicates high
  instability in the estimate (likely due to the denominator coefficient
  being close to zero) and implies that we cannot determine the
  direction of the effect with confidence.

- r.squared (High Predictive Power):  
  The 95% confidence interval for $r^2$ ranges from 0.894 to 0.928. This
  narrow and high interval shows that the model is highly robust. We can
  be 95% confident that the predictors explain between 89.4% and 92.8%
...
```
    ...
    ##    .id   term       estimate
    ##    <chr> <chr>         <dbl>
    ##  1 0001  r.squared     0.940
    ##  2 0001  beta_ratio -175.   
    ##  3 0002  r.squared     0.943
    ##  4 0002  beta_ratio -242.   
    ##  5 0003  r.squared     0.942
    ##  6 0003  beta_ratio -202.   
    ##  7 0004  r.squared     0.943
    ##  8 0004  beta_ratio -164.   
    ##  9 0005  r.squared     0.935
    ## 10 0005  beta_ratio -209.   
    ## # ℹ 9,990 more rows
...
The bootstrap distribution of $\hat{\beta}_{\text{ratio}}$ is unimodal
and left-skewed, with a peak approximately around -160. The values are
concentrated roughly between -300 and -100, with a tail extending
towards -400. Unlike the previous description, the distribution is
strictly negative and does not center around 0. This indicates a
consistent relationship where the ratio of the coefficients maintains a
negative sign, rather than oscillating between positive and negative
values.

Right Panel: $$r^2$$

The bootstrap distribution of $r^2$ is unimodal and approximately
symmetric, peaking around 0.942. The values are concentrated tightly
between 0.930 and 0.950. These consistently high values demonstrate that
the model explains about 93%–95% of the variation in maximum
temperature, indicating a very strong and robust goodness-of-fit.
...
knitr::kable(ci_results_all, digits = 3)
```

| term       | ci_lower | ci_upper |
|:-----------|---------:|---------:|
| beta_ratio | -280.843 | -125.065 |
| r.squared  |    0.935 |    0.947 |

- beta_ratio (Significant Negative Ratio):  
  The 95% confidence interval for
  $\hat{\beta}_{\text{ratio}} = \hat{\beta}_1 / \hat{\beta}_2$ falls
  between –280.843 and –125.065. Because this interval excludes zero,
  the ratio is statistically significant. The strictly negative range
  allows us to determine the direction of the effect with confidence,
  indicating a consistent relationship between the coefficients, unlike
  the high instability described previously.

- r.squared (High Predictive Power):  
  The 95% confidence interval for $r^2$ ranges from 0.935 to 0.947. This
  narrow and high interval shows that the model is highly robust. We can
  be 95% confident that the predictors explain between 93.5% and 94.7%
  of the variation in maximum temperature, demonstrating strong and
  stable predictive performance.
...
![](p8105_hw6_zn2220_files/figure-gfm/unnamed-chunk-14-1.png)<!-- -->
...
![](p8105_hw6_zn2220_files/figure-gfm/unnamed-chunk-15-1.png)<!-- -->
...
- Main Effects Model (Length + Gestational Age): This model has the
  ...
  poorest predictive performance among the three.

- Interaction Model (Head + Length + Sex interactions): This model
  performs significantly better than the main effects model, with a
  lower RMSE distribution .
