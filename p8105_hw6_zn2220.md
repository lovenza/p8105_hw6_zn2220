...
```
    ...
    ##    .id   term       estimate
    ##    <chr> <chr>         <dbl>
    ##  1 0001  r.squared     0.944
    ##  2 0001  beta_ratio -139.   
    ##  3 0002  r.squared     0.942
    ##  4 0002  beta_ratio -212.   
    ##  5 0003  r.squared     0.939
    ##  6 0003  beta_ratio -188.   
    ##  7 0004  r.squared     0.942
    ##  8 0004  beta_ratio -164.   
    ##  9 0005  r.squared     0.942
    ## 10 0005  beta_ratio -152.   
    ## # ℹ 9,990 more rows
...
The bootstrap distribution of $\hat{\beta}_{\text{ratio}}$ is
approximately unimodal with a distinct left skew and is centered around
–170. Most estimates fall between –100 and –300, and the entire
distribution is strictly negative. This indicates that the two
predictors have effects in opposite directions. The heavy left tail
suggests that for some bootstrap samples, the denominator (the
precipitation coefficient) is close to zero, causing the ratio to become
large and negative.

Right Panel: $$r^2$$

The bootstrap distribution of $r^2$ is highly concentrated, peaking
around 0.941, with a left tail extending to roughly 0.930 and an upper
cutoff near 0.950. These consistently high values demonstrate that the
model explains about 93%–95% of the variation in maximum temperature,
...
- beta_ratio (Significant Negative Ratio):  
  ...
  consistently have effects in opposite directions (specifically,
  minimum temperature has a positive effect while precipitation has a
  negative effect).
